...
class FreeTest extends WordSpecLike with Matchers {
  "Free" should {
    ...
    "work with turtle position with State" in {
      ...
      object TSW {
        def tell(xs: List[String]): TurtleStateWriter[Unit] = WriterT.tell(xs)
        def lift[A](fa: TurtleState[A]): TurtleStateWriter[A] = WriterT.lift(fa)
      }
      ...
      class MovementToWriterTState extends Natural[Movement, TurtleStateWriter] {
        override def transform[A](a: Movement[A]): TurtleStateWriter[A] = a match {
          case Start(pos) => for {
            _ <- TSW.tell(List("starting at Position(10,10)"))
            x <- TSW.lift(State.put(pos))
          } yield x
          case MoveUp(d) => for {
            _ <- TSW.tell(List(s"moving up $d steps"))
            x <- TSW.lift(State.modify[Position](p => p.copy(y = p.y + d)))
          } yield x
          case MoveDown(d) => for {
            _ <- TSW.tell(List(s"moving down $d steps"))
            x <- TSW.lift(State.modify[Position](p => p.copy(y = p.y - d)))
          } yield x
          case MoveLeft(d) => for {
            _ <- TSW.tell(List(s"moving left $d steps"))
            x <- TSW.lift(State.modify[Position](p => p.copy(x = p.x - d)))
          } yield x
          case MoveRight(d) => for {
            _ <- TSW.tell(List(s"moving right $d steps"))
            x <- TSW.lift(State.modify[Position](p => p.copy(x = p.x + d)))
          } yield x
        }
      }
